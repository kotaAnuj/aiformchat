 ...
 * a chat endpoint to dynamically generate resources, endpoints to generate, edit,
 * and update linked sheets, and a webhook to process form submissions.
 * 
 * Required packages:
...
/**
 * Creates a Google Sheet with a default table structure and returns its URL.
 * Automatically grants the specified user access to the sheet.
 */
async function createGoogleSheet(sheetTitle, userEmail) {
  try {
    ...
    // Write a default header row to the first sheet ("Sheet1") to create a table-like structure
    // You can adjust the header values as needed.
    ...
    // Optionally, you could add more formatting or additional sheets here.
    
    // Auto-grant permission to the user
    if (userEmail) {
      const drive = google.drive({ version: 'v3', auth });
      await drive.permissions.create({
        fileId: sheetId,
 ...
 * Currently, this function serves as a placeholder where you can add logic
 * to update the linked sheet if needed. By default, Google Forms auto-update the
 * linked Sheet if configured via the UI.
 */
async function linkFormToSheet(formId, sheetId) {
  try {
    logger.info(`Linking Form ${formId} to Sheet ${sheetId}`);
    // Additional linking logic could be added here if needed.
    return true;
  } catch (error) {
...
/**
 * Logs form (or sheet) creation details into Firestore.
 */
 ...
 *                 items:
 *                   type: array
 *                   description: Each inner array represents a row of data.
 *               userEmail:
 *                 type: string
 *     responses:
 *       200:
 ...
 *     description: This endpoint can be called by an external trigger (like a Google Apps Script)
 *                  to perform additional updates on the linked sheet when a form is submitted.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *               responseData:
 *                 type: array
 *                 description: The new row data from the form submission.
 *     responses:
 *       200:
 *         description: Form submission processed and sheet updated successfully.
 *       400:
 *         description: Invalid request.
 *       500:
 *         description: Server error.
